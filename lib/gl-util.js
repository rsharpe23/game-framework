export default gl => ({
  ...
  drawElements(buffer) {
    // Нужно доп. привязать буфер индексов перед отрисовкой, т.к. в 
    // примерах, в книге, это выполняется (стр. 97), и упоминается здесь 
    // webglfundamentals.org/webgl/lessons/webgl-indexed-vertices.html
